...
public class DiochanPostsParser implements GroupParser.Callback {
    ...
    private void updateConfiguration() {
        if (mHasPostBlock) {
            mConfiguration.storeNamesSpoilersEnabled(mBoardName, mHasPostBlockName, mHasSpoilerCheckBox);
        }
    }
    ...
    public boolean onStartElement(GroupParser parser, String tagName, String attrs) {
        if ("div".equals(tagName)) {
                ...
                if (src != null) {
                    if (src.endsWith("/css/sticky.gif")) {
                        ...
                        if ("100".equals(width) && "100".equals(height) && originalName != null &&
                            ...
                            attachment.setThumbnailUri(mLocator, mLocator.buildPath(path));
                        }
                    }
                }
            }
            ...
            if (mHasPostBlock) {
                ...
                if ("name".equals(name)) {
                    mHasPostBlockName = true;
                } else if ("spoiler".equals(name)) {
        ...
        switch (mExpect) {
            ...
            case EXPECT_COMMENT: {
                ...
                if (text.startsWith("<span style=\"float: left;")) {
                    ...
                    if (index >= 0) {
                        ...
                        if (index >= 0) {
                            embed = embed.substring(index + 5, embed.indexOf('"', index));
                        }
                    }
                ...
                if (mAttachments.size() > 0) {
                    for (int i = mAttachments.size() - 1; i >= 0; i--) {
                        if (mAttachments.get(i).getFileUri(mLocator) == null) {
            ...
            case EXPECT_OMITTED: {
                ...
                if (matcher.find()) {
                    ...
                    if (matcher.find()) {
                        mThread.addPostsWithFilesCount(Integer.parseInt(matcher.group(1)));
                    }
                }
                break;
            }
