...
public class DvachChanPerformer extends ChanPerformer {
        ...
        if ("3".equals(apiResult)) {
            ...
            if (id != null) {
                ...
                if (DvachChanConfiguration.CAPTCHA_TYPE_RECAPTCHA_1.equals(captchaType)
                    ...
                    OUTER:
                    if (DvachChanConfiguration.CAPTCHA_TYPE_2CHAPTCHA.equals(captchaType)) {
                        result = new ReadCaptchaResult(CaptchaState.CAPTCHA, captchaData);
    ...
    public SendChangePostRatingResult onChangePostRating(SendChangePostRatingData data) throws HttpException, InvalidResponseException {
        ...
        HttpResponse response = new HttpRequest(uri, data).addCookie(buildCookiesWithCaptchaPass()).read();
        JSONObject jsonObject = response.getJsonObject();
        Log.d("LIKES HTTP RESPONSE", jsonObject.toString());

        try {
            String resultMessage = (jsonObject.has("Status") ? jsonObject.get("Status") : jsonObject.get("Reason")).toString();

            return new SendChangePostRatingResult(true, resultMessage);
        } catch (JSONException e) {
            throw new InvalidResponseException(e);
        }
    }
